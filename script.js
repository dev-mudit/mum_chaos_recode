...
rickshawImg.loaded = false;
rickshawImg.onload = () => rickshawImg.loaded = true;
rickshawImg.onerror = () => rickshawImg.loaded = false;
rickshawImg.src = 'auto.png';

//PHASES
const PHASES = [
  {name:'Morning Rush',    minD:0,    spd:1.0, den:1.0, wx:'clear', sky:['#0d0d20','#1a0a05']},
  {name:'Afternoon Rain',  minD:300,  spd:1.3, den:2.2, wx:'rain',  sky:['#050418','#060400']},
  {name:'Evening Frenzy',  minD:700,  spd:1.7, den:3.2, wx:'heavy', sky:['#020210','#010100']},
  {name:'Midnight Mayhem', minD:1350, spd:2.2, den:4.8, wx:'chaos', sky:['#000008','#000000']},
];
...
function drawPuddles(){
  const off=S.roadY%260;
  [[LC[0]-10,off],[LC[2]+5,off+120]].forEach(([px,py])=>{
    const ry=py%H;
    ctx.fillStyle='rgba(40,80,160,.2)';ctx.beginPath();ctx.ellipse(px,ry,22,9,0,0,Math.PI*2);ctx.fill();
    ctx.strokeStyle='rgba(80,140,255,.25)';ctx.lineWidth=1;
    ctx.beginPath();ctx.moveTo(px-10,ry-2);ctx.lineTo(px+10,ry-2);ctx.stroke();
  });
}
...
function drawCar(o){
  const {x,y,col,tp}=o;
  ctx.fillStyle='rgba(0,0,0,.22)';ctx.beginPath();ctx.ellipse(x,y+26,16,6,0,0,Math.PI*2);ctx.fill();
  ctx.fillStyle=tp==='taxi'?'#FFD700':tp==='auto'?'#FFCC00':(col||'#e05555');
  ctx.fillRect(x-14,y-20,28,45);
  ctx.fillStyle='#2a2a2a';ctx.fillRect(x-10,y-14,20,22);
  ctx.fillStyle='rgba(150,200,255,.5)';ctx.fillRect(x-8,y-12,16,7);ctx.fillRect(x-8,y+5,16,7);
  ctx.fillStyle='#111';
  [[-12,-13],[12,-13],[-12,16],[12,16]].forEach(([wx,wy])=>{ctx.beginPath();ctx.arc(x+wx,y+wy,5.5,0,Math.PI*2);ctx.fill();});
  ctx.fillStyle='#FF4444';ctx.fillRect(x-14,y+20,5,4);ctx.fillRect(x+9,y+20,5,4);
  if(tp==='taxi'){ctx.fillStyle='#111';ctx.font='bold 7px sans-serif';ctx.textAlign='center';ctx.fillText('TAXI',x,y+3);}
}

function drawBus(o){
  const {x,y}=o;
  ctx.fillStyle='rgba(0,0,0,.28)';ctx.beginPath();ctx.ellipse(x,y+43,18,7,0,0,Math.PI*2);ctx.fill();
  ctx.fillStyle='#cc2211';ctx.fillRect(x-18,y-38,36,84);
  ctx.fillStyle='#aa1000';ctx.fillRect(x-18,y-43,36,7);
  ctx.fillStyle='rgba(150,200,255,.4)';
  [-6,6].forEach(i=>ctx.fillRect(x+i-5,y-28,10,12));
  ctx.fillStyle='#fff';ctx.font='bold 7px sans-serif';ctx.textAlign='center';ctx.fillText('BEST',x,y-6);
  ctx.fillStyle='#ffcc00';ctx.font='bold 9px sans-serif';ctx.fillText('321',x,y+8);
  ctx.fillStyle='#111';
  [[-12,-23],[12,-23],[-12,28],[12,28]].forEach(([wx,wy])=>{ctx.beginPath();ctx.arc(x+wx,y+wy,6,0,Math.PI*2);ctx.fill();});
  ctx.fillStyle='#FF4444';ctx.fillRect(x-18,y+38,7,4);ctx.fillRect(x+11,y+38,7,4);
}


function drawBike(o){
  const {x,y}=o;
  ctx.fillStyle='rgba(0,0,0,.18)';ctx.beginPath();ctx.ellipse(x,y+20,9,3.5,0,0,Math.PI*2);ctx.fill();
  ctx.strokeStyle='#777';ctx.lineWidth=2.5;
  ctx.beginPath();ctx.moveTo(x-7,y+2);ctx.lineTo(x,y-13);ctx.lineTo(x+7,y+2);ctx.stroke();
  ctx.strokeStyle='#555';ctx.lineWidth=2;
  [-10,10].forEach(dy=>{ctx.beginPath();ctx.arc(x,y+dy,8.5,0,Math.PI*2);ctx.stroke();});
  ctx.fillStyle='#555';ctx.beginPath();ctx.arc(x,y-20,5.5,0,Math.PI*2);ctx.fill();
  ctx.fillStyle='#333';ctx.fillRect(x-3,y-15,6,12);
}
